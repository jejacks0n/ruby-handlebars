  ...
  context 'recognizes' do
    ...
    it 'simple replacements' do
      expect(parser.parse('{{plic}}')).to eq(
        block_items: [
          base.merge(replaced_unsafe_item: 'plic')
        ]
      )
      ...
      expect(parser.parse('{{~#if test~}}foo{{/if}}')).to eq(
        block_items: [
          {
            block_items: [
            ...
            collapse_before: '~',
            helper_name: "if",
            parameters: { parameter_name: "test" }
          }
        ]
      )
    end
    ...
    it 'special variables' do
      expect(parser.parse('{{@first}}')).to eq(
        block_items: [
          base.merge(replaced_unsafe_item: '@first')
        ]
      )
      expect(parser.parse('{{@last}}')).to eq(
        block_items: [
          base.merge(replaced_unsafe_item: '@last')
        ]
      )
      expect(parser.parse('{{@index}}')).to eq(
        block_items: [
          base.merge(replaced_unsafe_item: '@index')
        ]
      )
    end
    ...
    it 'safe strings' do
      expect(parser.parse('{{{plic}}}')).to eq(
        block_items: [
          base.merge(replaced_safe_item: 'plic')
        ]
      )
    ...
    it 'comments' do
      expect(parser.parse('{{! this is a comment }}')).to eq(
        block_items: [
          base.merge(comment: ' this is a comment ')
        ]
      )
    end
        ...
        expect(parser.parse('{{#each people}} {{this.name}} {{/each}}')).to eq(
          block_items: [
            base.merge(
              ...
              block_items: [
                {template_content: ' '},
                base.merge(replaced_unsafe_item: 'this.name'),
                {template_content: ' '}
              ]
            )
          ]
            ...
            base.merge(
              ...
              block_items: [
                ...
                base.merge(
                  ...
                  block_items: [
                    {template_content: ' <li>'},
                    base.merge(replaced_unsafe_item: 'this'),
                    {template_content: '</li> '}
                  ]
                ),
                {template_content: '</ul>'}
    ...
    context 'templates with single curlies' do
      ...
      it 'works with closing curly before value' do
        expect(parser.parse('Hi }{{ hey }}')).to eq(
          block_items: [
            {template_content: 'Hi }'},
            base.merge(replaced_unsafe_item: 'hey')
          ]
        )
      end
